...
public class ChessMatch {
	...
	public ChessPiece replacePromotedPiece(String type) {
		...
		if (!type.equals("B") && !type.equals("N") && !type.equals("R") && !type.equals("Q") &&
		    !type.equals("b") && !type.equals("n") && !type.equals("r") && !type.equals("q")) {
	...
	private boolean testCheckMate(Color color) {
		...
		for (Piece p : list) {
			...
			for (int i = 0; i < board.getRows(); i++) {
				for (int j = 0; j < board.getColumns(); j++) {
					if (mat[i][j]) { // se alguma posição possível de movimentos, tirar o check, deve retornar falso.
						Position source = ((ChessPiece) p).getChessPosition().toPosition();
						Position target = new Position(i, j);
						Piece capturedPiece = makeMove(source, target);
						boolean testCheck = testCheck(color);
						undoMove(source, target, capturedPiece); // este é um movimento de teste, deve ser desfeito antes de
																											// terminar o teste.
						if (!testCheck) {
							return false;
						}
